...
export class DialogService {
  ...
  public openDialog(
    ...
    if (onEmitEvent) {
      ...
      // setTimeout(() => {
      for (const key in modalConfig.inputs) {
        if (Object.prototype.hasOwnProperty.call(instance, key)) {
          instance[key] = modalConfig.inputs[key];
        }
      }
      for (const keyName in instance) {
        if (instance[keyName] instanceof EventEmitter) {
          instance[keyName].subscribe((value: any) => {
      ...
      modal.afterOpen.subscribe((result) => {
        return onEmitEvent('afterOpen', result);
      });
      modal.afterClose.subscribe((result) => {
        return onEmitEvent('afterClose', result);
      });
    }
    return dialogData;
  }
